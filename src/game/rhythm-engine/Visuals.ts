...
export class VisualManager {
	...
	private readonly UR_MAX_MS = 150; // fps counter
	...
	private setupURBar() {
		...
		const hitY = height * CONSTANTS.HIT_POSITION_RATIO - 200;

		const bg = new PIXI.Graphics(); // Função auxiliar para converter MS em largura de pixels na barra

		const msToPx = (ms: number) => (ms / this.UR_MAX_MS) * (this.UR_WIDTH / 2); // 1. Zona de GOOD (Azul - a maior área externa)
	...
	private drawHitLine() {
		...
		this.hitLine.clear(); // Linha de base escura (estética)
	...
	private drawReceptor(g: PIXI.Graphics, col: number, active: boolean) {
		...
		const w = this.columnWidth - padding * 2; // Cores: Cinza escuro para idle, Branco puro para ativo
		...
		if (active) {
			...
			g.rect(-2, -2, w + 4, 34).fill({ color: 0xc3c3c3, alpha: 0.15 }); // Borda branca sólida
	...
	public render(notes: HitObject[], currentTime: number, ve: VelocityManager) {
		...
		const padding = this.columnWidth * 0.1; // azul rosa rosa azul
		...
		notes.forEach((note) => {
			...
			if (yPos > -200 && yEnd < height + 200) {
				...
				if (note.type === "TAP") {
					...
					const bodyH = Math.max(0, headY - yEnd); // Corpo da LN com alpha para transparência neon
	...
	private animateText() {
		for (let i = this.activeAnimations.length - 1; i >= 0; i--) {
			...
			const txt = anim.text; // 1. Interpolação da Escala // O texto "esmaga" até o tamanho alvo

			txt.scale.x += (1 - txt.scale.x) * 0.2;
			txt.scale.y += (1 - txt.scale.y) * 0.2; // 2. Movimento e Alpha

			txt.alpha -= 0.025; // 3. Destruição
	...
	public showJudgement(txt: string, color: number, yOffset?: number) {
		...
		j.y = this.app.screen.height * 0.45 + (yOffset || 0); // Um pouco acima do centro // 2. Efeito de Escala (Pop-in) // Começa grande e diminui rapidamente para o tamanho normal
	...
	public showLightning(col: number) {
		...
		this.effectContainer.addChild(l); // Armazena a referência usando a coluna como chave
